...
namespace environment
{
	...
	// similar to colliding function above but fix was required for falling state
	bool Collider::sideWall(srac::playerDataRef player, sf::Vector2f movement)
	{
		// top left point of character (NOT of texture/sprite!) + movement
		sf::Vector2i playerPosition = sf::Vector2i(player->attributes.getPosition()) - _playerOrigin + toVec2i(movement);
		sf::Vector2i playerSize = sf::Vector2i(CHARACTER_WIDTH, CHARACTER_HEIGHT);

		if (collisions(Box(playerPosition, playerSize)))
		{
			std::vector<Box> boxes = collidedBoxes(Box(playerPosition, playerSize));

			if (boxes.size() == 1)
			{
				Box collider = boxes[0];

				if (player->attributes.getPosition().x < collider.left)
				{
					player->attributes.setPosition(collider.left - 1.0f, player->attributes.getPosition().y);
				}
				else if(player->attributes.getPosition().x > collider.left + collider.width)
				{
					player->attributes.setPosition(collider.left + collider.width + 1.0f, player->attributes.getPosition().y);
				}
			}
